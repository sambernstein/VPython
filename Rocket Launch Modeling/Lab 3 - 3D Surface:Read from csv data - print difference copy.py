...
b = -0.29375 # air resistance constant # FIND BY EXPERIMENT
g = vector (0,-9.8,0) # gravitational constant
k = 214.0 # spring constant of launch setup # FIND BY EXPERIMENT
...
start_h = 1 # height between bottom horizontal launch bar and bottom of rubber bands in loaded position ------------MEASURE # in inches
...
exp_file = input("Enter CSV exp. data with 3 columns (angle, notch, distance) file directory, in quotation marks:")
...
##                sphere(radius=.03, color = color.green, pos = rocket.pos)
    ...
    ##                ball.pos = rocket.pos[0] + vector(0,1,0)
            ...
            diff[ind_pair[n]] = data_sim[ind_pair[n]] - data_exp[ind_pair[n]]

            if data_exp[ind_pair[n]] != -1:                
                print ind_pair[n], data_sim[ind_pair[n]], "    -    ", ind_pair[n], data_exp[ind_pair[n]], "        ", diff[ind_pair[n]]
                point_exp[n] = sphere(color=color.yellow, pos = (ind_pair[n][0], f*data_exp[ind_pair[n]], h_graph*ind_pair[n][1]), radius = point_r)

            else:
                print ind_pair[n], data_sim[ind_pair[n]], "    -    ", ind_pair[n], data_exp[ind_pair[n]], "" 
