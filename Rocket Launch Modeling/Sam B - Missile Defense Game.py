...
# video screen
screen = display(x = 0, width = 600, height = 600, center = (0,2,0), title = "Game")
...
c = controls(x = screen.x, y = screen.height + 5, width = screen.width, height = 200, title = "Controls")
...
line.append(text(text="from enemy missiles", pos = (-floor.length*.5 + line_set, floor.height*.5 + 1, 0), height = t_size, color = color.red))
...
g = vector (0,-40,0) # gravitational constant
...
##            print "t = ", t
        ...
        if t%rescue_t == 0:
            ...
            if empty_bases:
                ...
                rescue.append(pyramid(size = (.25,.25,.25), pos = (5*(random.random()-.5), 4+1.5*random.random(), 0), axis = (0,1,0), up = (1,0,0), opacity = .5, color= color.magenta))
